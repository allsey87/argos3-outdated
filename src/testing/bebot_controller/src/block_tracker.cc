...
void CBlockTracker::AssociateAndTrackTargets(std::chrono::time_point<std::chrono::steady_clock> t_timestamp,
   ...
   for(const auto& t_assignment : vecAssignmentIndices) {
      bool bBlockExists = (t_assignment.first < static_cast<int>(t_unassociated_block_list.size()));
      bool bTargetExists = (t_assignment.second < static_cast<int>(t_target_list.size()));
      if(bBlockExists) {
